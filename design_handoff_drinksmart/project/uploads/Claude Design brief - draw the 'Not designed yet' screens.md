...
The DrinkSmart handoff bundle covers designs `1a`–`1k`, but its own README ends with a  
**"Not designed yet"** list: Profile / onboarding, drink picker, menu scanner, establishment  
browsing, and auth — roughly half the app. It offers *"they follow the same tokens — ask and*  
*they'll be drawn."* This is that ask.

**Why it's on the critical path.** The locked precedence ladder ranks Claude Design entities above  
all other sources; anything not drawn falls to rank 6, implementer judgement. The whole-app redesign  
reflows every screen the moment the shared primitives change, so these screens will be visibly  
broken with no drawing to build them back from. Drawing them first is what keeps them off invented  
values.

It also asks for one thing the bundle never covered: **form-control primitives**. The design drew 5  
primitives; the app ships 51. Inputs, selects and dialogs inherit the new colours automatically  
through the CSS variables, but keep shadcn's sub-56px heights — so a 64px button currently sits  
above a 40px input.
...
- Unpack alongside the existing bundle; the precedence ladder already governs how it beats the  
current code.
- **A new export is expected to contradict what is already built.** That is the mechanism, not a  
defect — a Claude Design edit supersedes the implementation.
- Section A unblocks the deferred **W1-C** (form controls to the touch scale), which is currently  
held back specifically to avoid inventing values.
- Sections B–G unblock **Wave 4**, which has no drawings at all today.
