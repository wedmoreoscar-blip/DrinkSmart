...
- [ ] `OPENROUTER_API_KEY` set as Supabase secret
...
- **OpenRouter API abuse.** If your `OPENROUTER_API_KEY` leaks (e.g., committed to git), you'll get a real bill before you notice. Mitigations: rotate keys regularly, use OpenRouter's per-key spend limits, never reference `OPENROUTER_API_KEY` outside the edge functions (the client uses `supabase.functions.invoke`, never direct OpenRouter). Currently we follow this rule.
