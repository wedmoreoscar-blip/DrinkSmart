...
- **Every count on this list goes stale.** Derive them by running the command when writing a spec;
  never quote this file. The lint count sat at 9 here while the real figure was 10, and a spec
  written from it would have licensed a regression as "baseline held".
...
6. **`user_sessions` still exists in the schema but nothing in the client touches it** (verified 2026-08-16: no read or write outside a migration test). It is one row per user, PK = `user_id`, upsert pattern, with the `handle_user_sessions_updated_at` trigger. The live "use last night" path is **`user_session_history`** via `src/hooks/useSessionHistory.ts` — a different table with different semantics: **one immutable row per completed session, inserted and never upserted, and account-only.** `historyAccountUserId` returns null for an anonymous session, so anonymous users neither read nor write history. Do not conflate the two, and do not assume `user_sessions` is dead until someone checks whether the AI or a report reads it server-side.
...
20. **`generatePlan` never throws.** It wraps the edge function call in a 22s timeout race (`EDGE_FUNCTION_TIMEOUT_MS`); on any failure it falls back to `greedyPlanFallback`. Callers don't need try/catch. The return type is `GeneratePlanResult` which has a `usedFallback: boolean` flag — surface that to the user if they're suddenly getting deterministic plans.
...
22. **The model is `deepseek/deepseek-v4-flash-0731`** through OpenRouter's OpenAI-compatible API. To change it, update `DEEPSEEK_MODEL` at the top of `generate-plan/index.ts`. **Routing is `only: ["coreweave", "wafer"]`, CoreWeave preferred, not DeepSeek** (2026-08-16): a jurisdiction guardrail on the OpenRouter account blocks every CN-headquartered provider, so DeepSeek's own endpoint 404s and no privacy/ZDR setting overrides it. Both hosts serve identical weights and both scored 30/30 through the ±10% gate over 30 trials; CoreWeave has the tighter tail (p50 3.3s / p90 5.1s / max 8.0s vs 4.0 / 8.2 / 16.0) and declares fp8. `allow_fallbacks` is `true` **only between these two** — Wafer hit a 429 `server_overloaded` from a shared upstream pool during benchmarking, so a single pin makes one provider's capacity a hard outage. Don't add a host without re-running `or_bench`: DigitalOcean produced no tool call in 30 attempts, and Decart (an earlier pin) runs at 42 tok/s with a 21.9s tail.

22e. **Two tools, and the assistant echo must be sanitised.** `calculate_ethanol` earns its extra round trip: measured over 30 trials per configuration, with it 30/30 plans cleared the ±10% gate (worst deviation ±5%); without it 29/30 cleared (worst −16%), for ~1s and ~1.1 extra rounds. **Never push the raw assistant message back into `messages`** — OpenRouter attaches `reasoning`/`reasoning_content`, CoreWeave's deserializer treats them as one field, and replaying it 400s with `duplicate field reasoning_content` on every round after the first. Echo only `role`, `content` and `tool_calls`. Reasoning must be disabled as `reasoning: { enabled: false }`; `effort: "none"` is not a supported effort for this model and `effort: "low"` does not disable it (measured: 400 reasoning tokens vs 0).
...
22c. **Client tops up underfills as a safety net.** If the AI plan's actual ethanol is more than 15% under target, `topUpIfUnderfilled` in `src/lib/generatePlan.ts` bumps quantities on existing picks. The `calculate_ethanol` self-correction tool is gone (see 22), so the model's unaided arithmetic is all that stands between a plan and the server's ±10% gate — expect this to trigger more often. Note it only runs on plans the server **accepted**: a plan rejected as off-target returns 422 and the client falls straight to greedy, where top-up cannot help. Client timeout is 22s (`EDGE_FUNCTION_TIMEOUT_MS`), measured against a 1-round call at 13.8–21.7s for the OpenRouter leg alone.
...
src/hooks/
  useSessionHistory.ts   user_session_history snapshots — account-only, insert-only
